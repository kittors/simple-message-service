...
- [Node.js](https://nodejs.org/ "null") (v18 或更高版本)
    
- [pnpm](https://pnpm.io/installation "null") (v9 或更高版本)
    
- [Docker](https://www.docker.com/get-started "null") 和 Docker Compose
...
1. **复制环境变量文件**: 项目根目录下已经提供了 `.env.dev` 文件，无需手动创建。
    
2. **启动开发数据库**: 项目使用 Docker Compose 来管理开发环境的 PostgreSQL 数据库。
...
3. **初始化数据库表**: 数据库启动后，需要初始化 `messages` 表结构。
...
3. 使用 `tsc` 编译后端 TypeScript 代码。
    
4. 自动将生产环境所需的 `Dockerfile`, `docker-compose.yml` 和 `.env.prod` 文件复制到 `dist` 目录。
    
5. 为 `dist/backend` 生成一个干净的 `package.json` 并安装生产依赖。
...
2. **配置生产环境变量**: `dist` 目录中已经包含了一个 `.env` 文件（由 `.env.prod` 复制而来）。你可以根据实际部署需求修改其中的数据库密码、端口等信息。
    
3. **启动服务**: 使用 Docker Compose 一键启动应用和数据库服务。
